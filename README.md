...
For 4-FSK, each possible 2-bit combination (00, 01, 10, 11) is represented by a unique frequency.  During modulation, the bit stream is processed in pairs, with each pair getting mapped to the corresponding frequency. During demodulation, the Goertzel algorithm calculates the energy at each of the four target frequencies, and the pair of bits corresponding to the frequency with the highest energy is selected.
